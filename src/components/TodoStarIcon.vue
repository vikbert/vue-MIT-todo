...
<script>
  ...
  export default {
    ...
    data() {
      return {
        iconStarred: this.todo.starred,
      };
    },
    props: ['todo', 'counterActiveStarred'],
    computed: {
      spanClassName: function() {
        return this.iconStarred ? (this.todo.completed ? 'starred starred_completed' : 'starred') : 'star';
      },
    },
    methods: {
      toggleStarStatus(todo) {
        if (3 == this.counterActiveStarred && todo.starred === 0) {
          this.showExceededNotification();
          return;
        }
        this.iconStarred = todo.starred === STATUS_STARRED ? STATUS_STAR : STATUS_STARRED;
        todo.starred = this.iconStarred;
      },
