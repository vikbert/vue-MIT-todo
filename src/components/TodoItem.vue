<template>
  <li class="todo" :class="{ completed: todo.completed, editing: todo === editedTodo }">
    <div class="view">
      ...
      <span v-if="todo.starred" @click="starClickHandler(todo)" class="starred">
      ...
      <span v-else @click="starClickHandler(todo)" class="star">
...
<script>
  export default {
    ...
    methods: {
      starClickHandler(todo) {
        todo.starred = todo.starred ? 0 : 1;
      },
